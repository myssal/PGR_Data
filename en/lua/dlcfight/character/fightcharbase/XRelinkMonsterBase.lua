...
function XRelinkMonsterBase:MonsterRunVarAwake()
    ...
    self:SoftFuryAwake() --技能测试变量初始化
end

---软狂暴运行初始化
function XRelinkMonsterBase:SoftFuryAwake()
    self.isHaveFury = false --是否有软狂暴
    self.enterFuryLevelTime = 360 --多少秒后进入软狂暴
    self.isOnFury = false --是否在软狂暴状态
    self.enterFurySkill = nil --如果有这个技能，在释放软狂暴的时候会执行一次。
end
...
--进入软狂暴
function XRelinkMonsterBase:OnEnterSoftFury()
