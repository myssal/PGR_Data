...
function XUiTheatre6BattleShopGridCommodity:RefreshTagHightLight()
    ...
    self.GridSkillUi:ShowTagHightLight(self._Control:CalcSkillHighlightTagsBySource(cfg, highlightSourceTagIds))
end
...
function XUiTheatre6BattleShopGridCommodity:RefreshBuyBtnStatus()
    ...
    if not self._Control:IsCoinEnough(tonumber(price) or 0) then
        showPrice = string.format(
            "<color=#%s>%s</color>", self._Control:GetClientConfigValue("ShopRefreshColor", CoinStatus.NoEnough), price
        )
    end
...
function XUiTheatre6BattleShopGridCommodity:RefreshBuildTag()
    if self.GridData.Type == ItemType.Skill then
        ...
        XUiHelper.RefreshCustomizedList(
            self.TagSkill.transform.parent,
            self.TagSkill,
            #buildTagCfgs,
            function (i, grid)
        ...
        XUiHelper.RefreshCustomizedList(self.TagRelic.transform.parent, self.TagRelic, #attrConfigs, function (i, grid)
...
function XUiTheatre6BattleShopGridCommodity:RefreshSellStatus()
    ...
    self.RImgGoldBg.gameObject:SetActiveEx(not self:IsSellOut())
