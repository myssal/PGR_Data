...
local HelpCourseTableKey = {
    ...
    BigWorldHelpCourseUnlockTrigger = {
...
function XBigWorldTeachConfigModel:GetBigWorldHelpCourseDetailIndexById(id)
    local config = self:GetBigWorldHelpCourseDetailConfigById(id)
...
function XBigWorldTeachConfigModel:GetUnlockTriggerDict()
    if self._TriggerDict then
        return self._TriggerDict
    end
    local dict = {}
    ---@type table<number, XTableBigWorldHelpCourseUnlockTrigger>
    local templates = self._ConfigUtil:GetByTableKey(HelpCourseTableKey.BigWorldHelpCourseUnlockTrigger)
    for _, template in pairs(templates) do
        if not dict[template.Type] then
            dict[template.Type] = {}
        end
        local typeDict = dict[template.Type]
        typeDict[template.TargetId] = template.HelpCourseId
    end
    self._TriggerDict = dict
