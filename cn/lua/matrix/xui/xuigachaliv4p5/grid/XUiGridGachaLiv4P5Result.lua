...
---@class XUiGridGachaLiv4P5Result : XUiNode
---@field Parent XUiGachaLiv4P5Show
local XUiGridGachaLiv4P5Result = XClass(XUiNode, "XUiGridGachaLiv4P5Result")

function XUiGridGachaLiv4P5Result:OnStart()
    self:InitUiObject()
end

function XUiGridGachaLiv4P5Result:InitUiObject()
...
function XUiGridGachaLiv4P5Result:SetData(rewardInfo)
    self.Reward = rewardInfo
    
    local id = 0

    if XTool.IsNumberValidEx(self.Reward.Id) then
        id = self.Reward.Id
    elseif XTool.IsNumberValidEx(self.Reward.TemplateId) then
        id = self.Reward.TemplateId
    else
        XLog.Error('结果奖励数据缺少有效的Id：', rewardInfo)
    end
...
function XUiGridGachaLiv4P5Result:LoadEffect(id)
...
function XUiGridGachaLiv4P5Result:SetBgRawImage(id)
...
return XUiGridGachaLiv4P5Result