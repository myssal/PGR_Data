---@class XUiGachaLiv4P5BuyTicket : XLuaUi
local XUiGachaLiv4P5BuyTicket = XLuaUiManager.Register(XLuaUi, "UiGachaLifu405BuyTicket")

function XUiGachaLiv4P5BuyTicket:OnAwake()
...
function XUiGachaLiv4P5BuyTicket:OnStart(gachaCfg, itemData1, itemData2, targetItemData, forceBuyCount, closeCb)
...
function XUiGachaLiv4P5BuyTicket:OnEnable()
    self:Refresh()
...
function XUiGachaLiv4P5BuyTicket:InitItemCardPanel(index)
    local panel = {}
...
function XUiGachaLiv4P5BuyTicket:InitButton()
...
function XUiGachaLiv4P5BuyTicket:Refresh()
    self:RefreshUiShow()
    self:RefreshBuyAmount(tonumber(self.TxtSelect.text))
end

function XUiGachaLiv4P5BuyTicket:GetItemData(index)
    return index == 1 and self._ItemData1 or self._ItemData2
end

function XUiGachaLiv4P5BuyTicket:RefreshUiShow()
...
function XUiGachaLiv4P5BuyTicket:RefreshBuyAmount(count)
    self._BuyAmount = count
end

function XUiGachaLiv4P5BuyTicket:CheckBuyAmountLimit(count)
...
function XUiGachaLiv4P5BuyTicket:OnSelfSetBuyCount(count)
...
function XUiGachaLiv4P5BuyTicket:OnBtnMaxClick()
...
function XUiGachaLiv4P5BuyTicket:OnBtnAddSelectClick()
...
function XUiGachaLiv4P5BuyTicket:OnBtnMinusSelectClick()
    local count = tonumber(self.TxtSelect.text)
    local targetCount = count - 1
    if self:CheckBuyAmountLimit(targetCount) then
        return
...
-- text改动的回调
function XUiGachaLiv4P5BuyTicket:OnSelectTextChange()
    local count = tonumber(self.TxtSelect.text)
...
function XUiGachaLiv4P5BuyTicket:OnImgBtnClick(index)
    local data = XDataCenter.ItemManager.GetItem(self:GetItemData(index).ItemId)
    XLuaUiManager.Open("UiTip", data)
end

function XUiGachaLiv4P5BuyTicket:OnBtnBuyClick(index)
    -- CD检测
...
function XUiGachaLiv4P5BuyTicket:CheckBuyCD()
...
return XUiGachaLiv4P5BuyTicket