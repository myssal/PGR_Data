...
---@class XUiGachaLiv4P5Result : XLuaUi
local XUiGachaLiv4P5Result = XLuaUiManager.Register(XLuaUi, "UiGachaLifu405Result")

local MODE_LOOP = 1

function XUiGachaLiv4P5Result:OnAwake()
    self:InitAutoScript()
end

function XUiGachaLiv4P5Result:OnStart(gachaId, rewardList, backCb, background)
...
function XUiGachaLiv4P5Result:OnEnable()
    XUiHelper.PopupFirstGet()
end

function XUiGachaLiv4P5Result:OnDisable()
    XDataCenter.AntiAddictionManager.EndDrawCardAction()
end

function XUiGachaLiv4P5Result:Update()
...
function XUiGachaLiv4P5Result:InitAutoScript()
    self._SpecialSoundMap = {}
    self:AutoAddListener()
end

function XUiGachaLiv4P5Result:GetAutoKey(uiNode, eventName)
    if not uiNode then
        return
    end
...
function XUiGachaLiv4P5Result:RegisterListener(uiNode, eventName, func)
    ...
    if func ~= nil then
        if type(func) ~= "function" then
            XLog.Error("XUiGachaLiv4P5Result:RegisterListener函数错误, 参数func需要是function类型, func的类型是" .. type(func))
        end
...
function XUiGachaLiv4P5Result:AutoAddListener()
    self._AutoCreateListeners = {}
    self:RegisterClickEvent(self.BtnBack, self.OnBtnBackClick)
end

function XUiGachaLiv4P5Result:OnBtnBackClick()
...
function XUiGachaLiv4P5Result:SetupRewards()
...
function XUiGachaLiv4P5Result:ShowResult()
...
--已经拥有的角色转换碎片过程
function XUiGachaLiv4P5Result:ShowTrans()
    local count = 0
    for i = 1, #self._RewardList do
        if self._RewardList[i].ConvertFrom ~= 0 then
            count = count + 1
