...
function XUiPanelTheatre6TopSan:OnStart()
...
function XUiPanelTheatre6TopSan:UpdateView()
    ...
    --    if string.IsNilOrEmpty(config.Color) then
    --        goto CONTINUE
    --    end
    --
    --    local angle = self:GetAngle(config.MinSan)
    --    local rad = angle * Mathf.Deg2Rad
    --    local x = self._Radius * Mathf.Sin(rad) + offsetX
    --    local y = self._CenterY - self._Radius * Mathf.Cos(rad)
    --
    --    local line = self._Lines[i]
    --    if not line then
    --        line = i == 1 and self.ImgLine or XUiHelper.Instantiate(self.ImgLine, self.ImgLine.parent)
    --        line = line:GetComponent("Image")
    --        self._Lines[i] = line
    --    end
    --
    --    line.color = XUiHelper.Hexcolor2Color(config.Color)
    --    line.transform.anchoredPosition = Vector2(x, y)
    --    line.transform.localRotation = CS.UnityEngine.Quaternion.Euler(0, 0, angle)
    --
    ...
    --        txtNum.transform.anchoredPosition = Vector2(txtX + self._SanTextOffsetX, txtY + self._SanTextOffsetY)
    --        txtNum.transform.localRotation = CS.UnityEngine.Quaternion.identity
