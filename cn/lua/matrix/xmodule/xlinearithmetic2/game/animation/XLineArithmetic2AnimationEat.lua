...
function XLineArithmetic2AnimationEat:SetData(gridUid, eatIndex, eatGridType)
...
function XLineArithmetic2AnimationEat:Update(game, ui, deltaTime)
    if self._State == XLineArithmetic2Enum.ANIMATION_STATE.NONE then
        ...
        --if self._EatGridType == XLineArithmetic2Enum.GRID.BULLET_FILL then
        --    self._Duration = 0.5
        --    local grid = ui:GetGrid(self._GridUid)
        --    if grid then
        --        grid:PlayAnimation("LineEnable")
        --    end
        --elseif self._EatGridType == XLineArithmetic2Enum.GRID.END_FILL then
        --    self._Duration = 0.5
        --    local grid = ui:GetGrid(self._GridUid)
        --    if grid then
        --        grid:PlayAnimation("ColourEnable")
        --    end
        --end
