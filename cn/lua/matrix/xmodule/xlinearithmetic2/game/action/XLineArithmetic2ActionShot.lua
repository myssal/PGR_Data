...
function XLineArithmetic2ActionShot:Execute(game, model)
    ...
    if grid:GetType() == XLineArithmetic2Enum.GRID.END_COLOR_THROUGH then
        ...
        if #grid2Colors > 0 then
            local duplicates = {}
            for i = 1, #grid2Colors do
                local grid2Color = grid2Colors[i]
                self:ColorAdjacentSameColorGrid(game, grid2Color, grid, model, duplicates)
            end
        end
...
---@param model XLineArithmetic2Model
function XLineArithmetic2ActionShot:ColorAdjacentSameColorGrid(game, grid, endGrid, model, duplicates)
    duplicates = duplicates or {}
    -- 认为grid是安全的, 如果grid非法, 那么这个函数gg
    local color = endGrid:GetBulletColor()
    local color2Replace = grid:GetColor()
    local pos = grid:GetPos()
    local queue = XQueue.New()
    local passed = {}
    queue:Enqueue(pos)
    local layerDict = { [grid:GetUid()] = 1 }
            ...
            for i = 1, 4 do
                ...
                if game:IsInMap(nextP) then
                    ...
                    if neighbour and neighbour:IsCanColor() and neighbour:GetColor() == color2Replace then
                        if not passed[neighbour] then
                            local layer = layerDict[grid2Color:GetUid()]
                            layerDict[neighbour:GetUid()] = layer + 1
    ...
    if game:IsAnimation() then
        ...
        for uid, layer in pairs(layerDict) do
            if not duplicates[uid] then
                ...
                layer2Animation[layer]:Add(animation)
            end
        end
