...
function XLineArithmetic2Control:UpdateGameMapData(mapData, game)
    local selectedGrid = {}
    ...
    local eatCount = game:GetEatCount(line)
    if firstGrid then
        ---@type XLineArithmetic2ControlMapData
    ...
    if lastGrid and lastGrid:IsEndGrid() then
        local capacity = lastGrid:GetCapacity()
        local index = 0
        for i = #line, 1, -1 do
            local grid = line[i]
    ...
    if game:IsReadyShot() then
        ...
        if shotGrid then
            ...
            if direction.x ~= 0 or direction.y ~= 0 then
                ...
                self:UpdateGameMapData(mapDataPreview, self._GamePreview)
...
function XLineArithmetic2Control:UpdateHelpGame()
