...
function XSoloReformAgency:GetTeam(stageId)
    local team = self._TeamDic[stageId]

    if not team then
        team = XTeam.New(self._Model:GetStageTeamKey(stageId))
        self._TeamDic[stageId] = team
    end
...
function XSoloReformAgency:PreFight(stage, teamId, isAssist, challengeCount)
    ...
    --本地编队    
    local team = self:GetTeam(stage.StageId)
