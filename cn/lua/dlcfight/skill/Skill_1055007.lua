...
---@class XSkill_1055007 : XSkillBase
local XSkill_1055007 = XDlcScriptManager.RegSkillScript(1055007, "XSkill_1055007", Base)

function XSkill_1055007:ScriptInit(isGainControl)
...
function XSkill_1055007:IsGroundStateMatched()
...
function XSkill_1055007:IsActionTimingMatched()
    return self._proxy:CheckActionTiming(self._uuid, self._requiredActionTimingType)
end

function XSkill_1055007:CheckCastCondition(actionId)
    -- 第一步：先继承 Base 的通用释放检查
    if not Base.CheckCastCondition(self, actionId, self._baseTimingId) then
        -- print("Skill_1055007 CheckCastCondition: Base.CheckCastCondition(actionId, 0) 未通过")
        return false
    end
