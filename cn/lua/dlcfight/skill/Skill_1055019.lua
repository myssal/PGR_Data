...
---@class XSkill_1055019 : XSkillBase
local XSkill_1055019 = XDlcScriptManager.RegSkillScript(1055019, "XSkill_1055019", Base)

function XSkill_1055019:ScriptInit(isGainControl)
    Base.ScriptInit(self, isGainControl)
    self._baseTimingId = 0
    -- 当前必须正在释放 whiteCombo4 的第 3 段
    self._whiteCombo4SkillIds = { 1055016, 1055017, 1055018, 1055019 }
    self._requiredWhiteComboIndex = 3
    -- 只能在地面释放，2 表示地面
...
function XSkill_1055019:IsActionTimingMatched()
    return self._proxy:CheckActionTiming(self._uuid, self._requiredActionTimingType)
end

function XSkill_1055019:IsAttribMatched()
...
function XSkill_1055019:CheckCastCondition(actionId)
    -- 第一步：先继承 Base 的通用释放检查
    if not Base.CheckCastCondition(self, actionId, self._baseTimingId) then
        -- print("Skill_1055019 CheckCastCondition: Base.CheckCastCondition(actionId, 0) 未通过")
        return false
    end
